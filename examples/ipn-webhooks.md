...
Use [webhook.php](webhook.php) for either channel. Both send the same JSON; configure each receiver with the correct secret.
...
Serve `webhook.php` through your HTTPS PHP application (PHP 7.4+ with PDO SQLite). It also works without Composer. Configure `WHOLLY_SIGNING_SECRET`, `WHOLLY_CALLBACK_PROJECT_ID` and `WHOLLY_CALLBACK_DB` in the server environment. The database must be outside the web root, in a private directory. Do not put secrets in query strings.

If adapting an older copy of this example, the inbox now uses `wholly_callback_inbox`, scoped by project, invoice and sequence. Preserve and migrate existing queued work/replay records before switching your worker. The example does not migrate or delete your previous `wholly_notifications` table.
...
```php
// After Webhook::parse() has verified the raw body:
$status = $notification->status();
$amountState = $notification->payload['amount_status'];
$invoiceTotal = $notification->payload['amount']; // "49.9", not USDC received
// In your durable worker, using the project's configured API host and token:
$detail = $client->getInvoice($configuredProjectId, $notification->invoiceId());
$invoice = $detail['data'];
