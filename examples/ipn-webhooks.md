...
Use [webhook.php](webhook.php) as a receiver and [notification.json](notification.json) as a synthetic version-2 example. These examples send no payments and contain no real credentials.
...
The following is pseudocode, not a drop-in receiver. Use the state-based worker with the supplied [webhook.php](webhook.php); use the event-based worker only with an inbox that preserves distinct signed event IDs.
...
```php
$client->listInvoicePayments($projectId, $invoiceId, ['limit' => 25, 'offset' => 0])
```
